...
import com.lpl.pig.study.annotation.NotRepeatSubmit;
import com.lpl.pig.study.base.ApiResponse;
import com.lpl.pig.study.model.AccessToken;
import com.lpl.pig.study.model.AppInfo;
import com.lpl.pig.study.model.TokenInfo;
import com.lpl.pig.study.model.UserInfo;
import com.lpl.pig.study.utils.MD5Util;
...
public class TokenController {
    ...
    public ApiResponse<AccessToken> apiToken(@RequestBody String appId, @RequestHeader("timestamp") String timestamp,
                                             @RequestHeader("sign") String sign) {
        Assert.isTrue(!StringUtils.isEmpty(appId) && !StringUtils.isEmpty(timestamp) && !StringUtils.isEmpty(sign),
                "参数错误");
        long reqeustInterval = LocalDateTime.now().toEpochSecond(ZoneOffset.of("+8"));
        Assert.isTrue((reqeustInterval - Long.valueOf(timestamp)) < 5 * 60 * 1000, "请求过期，请重新请求");

        // 1. 根据appId查询数据库获取appSecret
        AppInfo appInfo = new AppInfo("1", "123456");
    ...
    public static void main(String[] args) {
        ...
        String signString = timestamp + "1" + "123456";
